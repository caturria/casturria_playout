...
Deno.test("Validate correctness of Audio processing operations", async(t: Deno.TestContext) => {
    ...
    await t.step("Create one minute of pink noise at 48000Hz", async (_t: Deno.TestContext) => {
        const noiseSource = "anoisesrc=color=pink:duration=60";
        const filterDescription = `${noiseSource}[0],${noiseSource}[1],[0][1]amerge`;
        ...
        await expect(graph.open(filterDescription, 48000, 2, 48000, 2)).resolves.toBeUndefined();
        ...
        await expect(encoder.open("file:./pink.flac", 48000, 2, {})).resolves.toBeUndefined();
    ...
    await t.step("Verify that pink noise file has correct duration", async () => {
        ...
        await expect(decoder.open("file:./pink.flac", 48000, 2)).resolves.toBeUndefined();
        let duration = 0;
        while(true)
        {
    ...
    await t.step("Verify duration of pink noise file with resampling and mono mixdown", async () => {
        await using decoder = new Decoder();
        await expect(decoder.open("file:./pink.flac", 44100, 1)).resolves.toBeUndefined();
     let duration = 0;
     while(true)
     {
    ...
    await t.step("Filtergraph I/O correctness test", async () => {
        ...
        const filterDescription = `${noiseSource}[0],${noiseSource}[1],[0][1]amerge[2],[2]${mixer}`;
        await using decoder = new Decoder();
        await expect(decoder.open("file:./pink.flac", 48000, 2)).resolves.toBeUndefined();
        await using graph = new FilterGraph();
     await expect(graph.open(filterDescription, 48000, 2, 48000, 2)).resolves.toBeUndefined();
