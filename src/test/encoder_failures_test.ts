...
Deno.test("Encoder should fail fast on invalid configuration", async (_t: Deno.TestContext) => {
    ...
    await expect(encoder.open("bad://fail", 48000, 2, {})).rejects.toThrow("Unable to determine an appropriate output format");

    //This is definitely a bogus protocol identifier.
    await expect(encoder.open("bad://fail.mp3", 48000, 2, {})).rejects.toThrow("Protocol not found");

//Posix-specific no such file or directory error (revisit this if we ever target native Windows).
        await expect(encoder.open("file:/zz:/~/fail.mp3", 48000, 2, {})).rejects.toThrow("No such file or directory");

        //Inappropriate container / codec pair (gives vague "Invalid argument" error).
        await expect(encoder.open("file:./inappropriate_container_codec.ogg", 48000, 2, {codec: "libmp3lame"})).rejects.toThrow("Invalid argument");

        //This one leaves a junk file behind:
        await expect(Deno.remove("./inappropriate_container_codec.ogg")).resolves.toBeUndefined();

        //No such codec:
                await expect(encoder.open("file:./no_codec.ogg", 48000, 2, {codec: "libnosuchcodec"})).rejects.toThrow("Invalid value 'libnosuchcodec'");

                //JSON keys with incorrectly-typed values.
                await expect(encoder.open("file:./incorrectly_typed.ogg", 48000, 2, {outSampleRate: "five", outChannels: "two"})).rejects.toThrow("Problem while parsing a number");

                //Objects with circular references can't be serialized to JSON.
                const circular = {circle: {}};
                circular.circle = circular;
                await expect(encoder.open("file:./circular.ogg", 48000, 2, circular)).rejects.toThrow(TypeError);
...
Deno.test("Encoder should detect API abuse", async (_t: Deno.TestContext) => {
    await using encoder = new Encoder();
                ...
                await expect(encoder.encode(new Float32Array(128))).rejects.toThrow("This encoder has not yet been opened.");
    ...
    await expect(encoder.open("file:./test.mp3", 48000, 2, {})).resolves.toBeUndefined();

    //It's open, so another open call is improper API use.
    await expect(encoder.open("file:./another_test.wav", 44100, 1, {})).rejects.toThrow(BadResource);
...
//The close() call should be idempotent:
    ...
    await expect(encoder.encode(new Float32Array(1024))).rejects.toThrow(BadResource);

    //Nor should it accept another finalize command:
    await expect(encoder.finalize()).rejects.toThrow(BadResource);

    //It can't be opened again:
    await expect(encoder.open("file:./not_reusable.opus", 44100, 1, {})).rejects.toThrow(BadResource);
