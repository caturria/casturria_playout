...
Deno.test("Filtergraph should fail fast on invalid configuration", async (_t: Deno.TestContext) => {
  ...
  await expect(graph.open("London Bridge is falling down", 48000, 2, 48000, 2))
    .rejects.toThrow("Filter not found");

  //Valid specification but invalid I/O parameters:
  await expect(graph.open("amix=inputs=2", -1031, 2, 44100, 2)).rejects.toThrow(
    RangeError,
  );
  await expect(graph.open("amix=inputs=2", 44100.2468, 2, 44100, 2)).rejects
    .toThrow(TypeError);
  await expect(graph.open("amix=inputs=2", 44100, 2, -1031, 2)).rejects.toThrow(
    RangeError,
  );
  await expect(graph.open("amix=inputs=2", 44100, 2, 44100.2468, 2)).rejects
    .toThrow(TypeError);
  await expect(graph.open("amix=inputs=2", 44100, 255, 44100, 2)).rejects
    .toThrow(RangeError);
  await expect(graph.open("amix=inputs=2", 44100, 2.468, 44100, 2)).rejects
    .toThrow(TypeError);
  await expect(graph.open("amix=inputs=2", 44100, 2, 44100, 255)).rejects
    .toThrow(RangeError);
  await expect(graph.open("amix=inputs=2", 44100, 2, 44100, 2.468)).rejects
    .toThrow(TypeError);
...
Deno.test("Filtergraph should detect API abuse", async (_t: Deno.TestContext) => {
  ...
  await expect(graph.send(new Float32Array(32), 0)).rejects.toThrow(
    BadResource,
  );
  ...
  await expect(graph.open("amix=inputs=2", 48000, 2, 48000, 2)).resolves
    .toBeUndefined();
  ...
  await expect(graph.send(new Float32Array(32), 2.468)).rejects.toThrow(
    TypeError,
  );
  ...
  await expect(graph.open("amix=inputs=2", 48000, 2, 48000, 2)).rejects.toThrow(
    BadResource,
  );
});
